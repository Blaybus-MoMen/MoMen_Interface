#!/bin/bash

# 서버에서 실행되는 배포 스크립트
# docker-compose 기반으로 실행

set -e

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🚀 Momen 원격 서버 배포"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

# ── Docker 접근 확인 ────────────────────────────────────────
echo ""
echo "🔧 Docker 데몬 접근 확인..."
docker info >/dev/null 2>&1 || { echo "❌ Docker 접근 실패"; exit 1; }
echo "✅ Docker 접근 가능"

# ── 이미지 로드 (tar 파일이 있는 경우) ──────────────────────
if [ -f "momen-image.tar" ]; then
    echo ""
    echo "📦 Docker 이미지 로드 중..."
    docker load -i momen-image.tar
    rm -f momen-image.tar
    echo "✅ 이미지 로드 완료"
fi

# ── 환경 변수 파일 확인 ────────────────────────────────────
if [ -f "momen.env" ] && [ ! -f ".env" ]; then
    cp momen.env .env
    echo "📋 momen.env → .env 복사"
fi

if [ ! -f ".env" ]; then
    echo "❌ .env 파일이 없습니다. 배포를 중단합니다."
    exit 1
fi

# ── docker-compose.yml 확인 ────────────────────────────────
if [ ! -f "docker-compose.yml" ]; then
    echo "❌ docker-compose.yml 파일이 없습니다. 배포를 중단합니다."
    exit 1
fi

# ── 기존 컨테이너 정리 및 docker-compose 실행 ──────────────
echo ""
echo "🛑 기존 컨테이너 정리 중..."
docker-compose down 2>/dev/null || true

# 이름 충돌 방지
docker stop momen momen-api momen-redis 2>/dev/null || true
docker rm momen momen-api momen-redis 2>/dev/null || true

echo ""
echo "🚀 Docker Compose 실행 중..."
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
docker-compose up -d

# ── 배포 결과 확인 ──────────────────────────────────────────
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "📊 배포 결과 확인"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

echo "📦 실행 중인 컨테이너:"
docker-compose ps

echo ""
echo "📋 앱 컨테이너 로그 (최근 30줄):"
docker-compose logs --tail=30 app 2>&1

# 헬스 체크
echo ""
echo "🏥 헬스 체크 (30초 대기)..."
sleep 30

if docker ps | grep -q "momen-api"; then
    SERVER_IP=$(hostname -I | awk '{print $1}' 2>/dev/null || echo "100.50.98.194")
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "🎉 배포 완료!"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "🌐 애플리케이션: http://$SERVER_IP:8089"
    echo "📋 로그 확인: docker-compose logs -f app"
    echo "🛑 중지 명령: docker-compose down"
    echo "🔄 재시작 명령: docker-compose restart app"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
else
    echo "❌ 컨테이너가 실행되지 않았습니다"
    echo "📋 전체 로그:"
    docker-compose logs app 2>&1
    exit 1
fi
